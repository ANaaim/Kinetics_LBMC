    ...
    for ind_plateform in range(nb_plateform):
        ...
        for name_param in list_param:
            for name_analog in list_key_analog:
                if name_param.lower() in name_analog.lower() and string_plateforme in name_analog.lower():
                    list_name_analog.append(name_analog)

        if len(list_name_analog) > len(list_param):
            temp = [name for name in list_name_analog if ("fp" in name.lower() or "pf" in name.lower())]
        ...
        for name_global, param_in_data in zip(list_param, name_in_data):
            ...
            force_plateform_temp[final_test_name] = sign * analog_data[:, analogs_ind[param_in_data], :]
        ...
        Mz_CoP_temp[mask] = Mz_temp + Fy_temp * (Xor - X_CoP_temp[mask]) - Fx_temp * (Yor - Y_CoP_temp[mask])  # *mask
        ...
        force_plateform_temp["origin_plateform"] = np.tile(Or[:, np.newaxis], (1, force_plateform_temp["CoP"].shape[1]))
