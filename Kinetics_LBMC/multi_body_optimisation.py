...
def multi_body_optimisation(full_segment, full_model, max_step=50):
    ...
    for ind_segment, segment in enumerate(full_segment):
        Km[ind_marker * 3 : (ind_marker + len(segment.rm)) * 3, ind_segment * 12 : (ind_segment + 1) * 12, :] = np.tile(
            segment.get_Km(), (1, 1, nb_frame)
        )
        W[
            ind_marker * 3 : (ind_marker + len(segment.rm)) * 3, ind_marker * 3 : (ind_marker + len(segment.rm)) * 3, :
    ...
    while error_evolution > 10**-2 and step < max_step:
        ...
        for ind_segment, segment in enumerate(full_segment):
            ...
            Kr[6 * ind_segment : 6 * (ind_segment + 1), 12 * ind_segment : 12 * (ind_segment + 1), :] = segment.get_Kr()
            ...
            dKlambdardQ[
                12 * ind_segment : 12 * (ind_segment + 1), 12 * ind_segment : 12 * (ind_segment + 1), :
            ] = np.copy(temp_DQ)
        ...
        for ind_constraint, constraint in enumerate(full_model):
            ...
            phik_temp = constraint.get_phik(full_segment[ind_dist], full_segment[ind_prox])

            Kk_temp = constraint.get_Kk(full_segment[ind_dist], full_segment[ind_prox])

            lambda_k_temp = lambda_k[constraint_quantity : constraint_quantity + constraint.nb_constraint, 0, :]

            phik[constraint_quantity : constraint_quantity + constraint.nb_constraint, :, :] = np.copy(
                phik_temp[:, np.newaxis, :]
            )
            # Division of Kk and DklambakdQ
            Kk[
                constraint_quantity : constraint_quantity + constraint.nb_constraint,
                ind_prox * 12 : (ind_prox + 1) * 12,
                :,
            ] = Kk_temp[:, 12:24, :]
            Kk[
                constraint_quantity : constraint_quantity + constraint.nb_constraint,
                ind_dist * 12 : (ind_dist + 1) * 12,
                :,
            ] = Kk_temp[:, 0:12, :]

            dKlambdakdQ_temp = constraint.get_dKlambdakdQ(nb_frame, lambda_k_temp)

            dKlambdakdQ[
                ind_constraint * 12 : (ind_constraint + 2) * 12, ind_prox * 12 : (ind_prox + 1) * 12, :
            ] = dKlambdakdQ_temp[:, 12:24, :]
            dKlambdakdQ[
                ind_constraint * 12 : (ind_constraint + 2) * 12, ind_dist * 12 : (ind_dist + 1) * 12, :
            ] = dKlambdakdQ_temp[:, 0:12, :]
        ...
        # Error
        error_phik = np.mean(np.einsum("mnr,ndr->mdr", np.transpose(phik, (1, 0, 2)), phik))
        ...
        error_phir = np.mean(np.einsum("mnr,ndr->mdr", np.transpose(phir, (1, 0, 2)), phir))
        print("Error phim")
        print(error_phim)
        print("Error phir")
        print(error_phir)
        ...
        dFdX = np.zeros(
            (
                nb_segment * 12 + nb_constraint + 6 * nb_segment,
                12 * nb_segment + nb_constraint + 6 * nb_segment,
                nb_frame,
            )
        )
        temp_Km_W_phim = np.einsum("mnr,ndr->mdr", np.transpose(Km, (1, 0, 2)), W)
        F[0 : nb_segment * 12, :, :] = np.einsum("mnr,ndr->mdr", temp_Km_W_phim, phim) + np.einsum(
            "mnr,ndr->mdr",
            np.transpose(np.concatenate((Kk, Kr), axis=0), (1, 0, 2)),
        ...
        dFdX[0 : nb_segment * 12, 0 : nb_segment * 12, :] = Full_KmT + DKlambdaridQ_Total

        dFdX[0 : nb_segment * 12, nb_segment * 12 : nb_segment * 12 + nb_constraint, :] = np.transpose(Kk, (1, 0, 2))
        dFdX[0 : nb_segment * 12, nb_segment * 12 + nb_constraint :, :] = np.transpose(Kr, (1, 0, 2))

        dFdX[nb_segment * 12 : nb_segment * 12 + nb_constraint :, 0 : nb_segment * 12, :] = Kk
        dFdX[nb_segment * 12 + nb_constraint :, 0 : nb_segment * 12, :] = Kr
        ...
        error = np.sum(np.sqrt(np.einsum("mnr,ndr->mdr", np.transpose(F, (1, 0, 2)), F)), axis=2)
        ...
        error_phim = np.sum(np.sqrt(np.einsum("mnr,ndr->mdr", np.transpose(F_m, (1, 0, 2)), F_m)), axis=2)
        error_phim = np.sqrt(error_phim) / (phim.shape[0] / 3 * phim.shape[2])

        error_phik = np.sum(np.sqrt(np.einsum("mnr,ndr->mdr", np.transpose(F_k, (1, 0, 2)), F_k)), axis=2)
        error_phik = np.sqrt(error_phik) / (phik.shape[0] / 3 * phik.shape[2])

        error_phir = np.sum(np.sqrt(np.einsum("mnr,ndr->mdr", np.transpose(F_r, (1, 0, 2)), F_r)), axis=2)
...
def calculate_error(full_segment, full_model):
    ...
    for ind_segment, segment in enumerate(full_segment):
        ...
        dKlambdardQ[12 * ind_segment : 12 * (ind_segment + 1), 12 * ind_segment : 12 * (ind_segment + 1), :] = np.copy(
            temp_DQ
        )
    ...
    constraint_quantity = 0
    for ind_constraint, constraint in enumerate(full_model):
        phik_temp = constraint.get_phik(full_segment[ind_constraint], full_segment[ind_constraint + 1])
        Kk_temp = constraint.get_Kk(full_segment[ind_constraint], full_segment[ind_constraint + 1])
        lambda_k_temp = lambda_k[constraint_quantity : constraint_quantity + constraint.nb_constraint, 0, :]
        ...
        Kk[
            constraint_quantity : constraint_quantity + constraint.nb_constraint,
            ind_constraint * 12 : (ind_constraint + 2) * 12,
            :,
        ] = Kk_temp

        dKlambdakdQ[
            ind_constraint * 12 : (ind_constraint + 2) * 12, ind_constraint * 12 : (ind_constraint + 2) * 12, :
        ] = constraint.get_dKlambdakdQ(nb_frame, lambda_k_temp)
    ...
    # Error
    error_phik = np.mean(np.einsum("mnr,ndr->mdr", np.transpose(phik, (1, 0, 2)), phik))
    error_phim = np.mean(np.einsum("mnr,ndr->mdr", np.transpose(phim, (1, 0, 2)), phim))
    error_phir = np.mean(np.einsum("mnr,ndr->mdr", np.transpose(phir, (1, 0, 2)), phir))
    print("Error phim")
    print(error_phim)
    print("Error phir")
