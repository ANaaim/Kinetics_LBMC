...
class Joint:
    ...
    def __init__(self, segment_dist, seg_phi_dist, frq_acq, frq_cp, gravity_direction, unit_point):
        ...
        if unit_point == "mm":
            ...
            print("The unit " + unit_point + " cannot be considered for dynamic calculation. It should be m or mm.")

        Hg = np.zeros((4, 4, nb_frame))
        Hg[abs(gravity_direction), 3, :] = (gravity_direction / abs(gravity_direction)) * norm_gravity
    ...
    def projection_JCS(self, joint_center, segment_prox, segment_dist, rotation_seq):
        ...
        phi_projected = joint_center_mathomo.inv() * (self.phi_prox_origin * joint_center_mathomo.inv().transpose())
