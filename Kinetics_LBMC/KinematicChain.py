...
class KinematicChain:
    ...
    def __init__(
        ...
        name_joint,
        name_rotation,
        pos_moment_calculation,
        frame_moment_calculation,
        fpoint,
        fc,
        gravity_direction,
        unit_point,
        ...
        for i in range(len(name_joint) + 1):
            euler_glob[name_segment[i]] = np.rad2deg(r2mobile.zxy(list_segment[i].Tprox.T_homo))
        ...
        for i in range(len(name_joint)):
            homo_segment_rel[name_joint[i]] = homogenous2rotation(list_segment[i + 1].Tdist, list_segment[i].Tprox)
        ...
        for ind_joint in range(len(name_joint)):
            if ind_joint == 0:
                phi_joints[name_joint[ind_joint]] = Joint(
                    list_segment[ind_joint], phi_ext[ind_joint], fpoint, fc, gravity_direction, unit_point
                )
            else:
                phi_temp = phi_joints[name_joint[ind_joint - 1]].phi_prox_origin + phi_ext[ind_joint]
        ...
        if moment_projection == "JCS":
            for ind_joint in range(len(name_joint)):
                force[name_joint[ind_joint]], moment[name_joint[ind_joint]] = phi_joints[
                    name_joint[ind_joint]
                ].projection_JCS(
                    pos_moment_calculation[ind_joint].T_homo[0:3, 3, :],
                    list_segment[ind_joint],
                    list_segment[ind_joint + 1],
                    name_rotation[ind_joint],
                )
        else:
            for ind_joint in range(len(name_joint)):
                force[name_joint[ind_joint]], moment[name_joint[ind_joint]] = phi_joints[
                    name_joint[ind_joint]
                ].get_force_moment(pos_moment_calculation[ind_joint], frame_moment_calculation[ind_joint])
