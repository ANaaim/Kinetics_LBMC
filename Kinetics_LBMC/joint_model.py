...
class spherical_model:
    def __init__(
        ...
        if point_of_moment_calculus is None:
            ...
            self.point_of_moment_calculus = HomogeneousMatrix(X_glob, Y_glob, Z_glob, point_proximal)
        else:
        ...
        nv_temp_distal = np.mean(
            vnop_array(
                point_distal - segment_distal.rp,
                segment_distal.u,
        ...
        nv_temp_proximal = np.mean(
            vnop_array(
                point_proximal - segment_proximal.rp,
                segment_proximal.u,
    ...
    def get_phik(self, segment_distal, segment_proximal):
        phik = np.dot(self.nVproximal.T, segment_proximal.Q) - np.dot(self.nVdistal.T, segment_distal.Q)
        return phik
...
class universal_model:
    def __init__(
        ...
        if point_of_moment_calculus is None:
            ...
            Y_glob = np.tile(np.array([0, 1, 0])[:, np.newaxis], (1, nb_frame))
            Z_glob = np.tile(np.array([0, 0, 1])[:, np.newaxis], (1, nb_frame))
            self.point_of_moment_calculus = HomogeneousMatrix(X_glob, Y_glob, Z_glob, segment_proximal.rd)
        else:
            self.point_of_moment_calculus = point_of_moment_calculus
        ...
        if axe_distal == "u":
            ...
            axe_dist_calc = (segment_distal.rp - segment_distal.rd) / segment_distal.length_mean
        elif axe_distal == "w":
        ...
        if axe_proximal == "u":
            ...
            axe_prox_calc = (segment_proximal.rp - segment_proximal.rd) / segment_proximal.length_mean
        elif axe_proximal == "w":
    ...
    def get_dKlambdakdQ(self, nb_frame, lambda_k):
        ...
        for distal_temp in distal:
            for proximal_temp in proximal:
                dKlambdakdQ[
                    abs(distal_temp) * 3 : (abs(distal_temp) + 1) * 3,
        ...
        for proximal_temp in proximal:
            for distal_temp in distal:
                dKlambdakdQ[
                    abs(proximal_temp) * 3 : (abs(proximal_temp) + 1) * 3,
...
class hinge_model:
    def __init__(
        ...
        if theta_1 is None:
            ...
            self.theta_1 = theta_1
        if theta_2 is None:
            self.theta_2 = np.mean(np.arccos(np.sum(segment_proximal.w * segment_distal.u, axis=0)))
        else:
    ...
    def get_phik(self, segment_distal, segment_proximal):
        ...
        phik[0:3, :] = segment_proximal.rd - segment_distal.rp
        phik[3, :] = np.sum(
            segment_proximal.w * (segment_distal.rp - segment_distal.rd), 0
        ) - segment_distal.length_mean * np.cos(self.theta_1)
...
class constant_distance:
    def __init__(
        ...
        nVproximal = np.zeros((12, 3))
        if distance is None:
            self.distance = np.mean(np.linalg.norm((point_distal - point_proximal), axis=0))
        else:
    ...
    def get_phik(self, segment_distal, segment_proximal):
        phik_temp = np.dot(self.nVproximal.T, segment_proximal.Q) - np.dot(self.nVdistal.T, segment_distal.Q)
    ...
    def get_Kk(self, segment_distal, segment_proximal):
        ...
        Kk = np.zeros((3, 2 * 12))
        Kk[:, 0:12] = -2(
            np.dot(self.nVproximal.T, segment_proximal.Q) - np.dot(self.nVdistal.T, segment_distal.Q)
        )  # (self.nVdistal.T)
        Kk[:, 12:] = self.nVproximal.T
        Kk_final = np.tile(Kk[:, :, np.newaxis], (1, 1, nb_frame))
        return Kk_final
